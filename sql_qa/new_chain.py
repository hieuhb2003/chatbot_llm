...
LIMIT 1;
    ...
    def create_chain(self, llm, db):
        ...
        example_selector = SemanticSimilarityExampleSelector.from_examples(
            ...
            input_keys=["input"]  # Key to use for similarity comparison
        )
...
Dưới đây là một số ví dụ tương tự với câu hỏi của bạn:"""
        ...
        def extract(res: str) -> str:
            """Extract the SQL query from the result string, handling both XML tags and plain text"""
            # Look for SQL query within XML tags
            start_tag = "<sql_query>"
            end_tag = "</sql_query>"
            ...
            if start_tag in res and end_tag in res:
                ...
                query = res[start_index:end_index].strip()
                return query
            
            # Fallback to looking for direct SQL if no tags found
...
new_chain = ChainSQL().create_chain(llm, db)
chat_history = []
question = "Cho tôi biết tên sản phẩm thuộc loại Đèn Năng Lượng Mặt Trời có giá dưới 500000" 
