...
from csv2text.process_data import ProductVectorProcessor
import string
import os
...
class DocumentRetrieval:
    ...
    def __init__(self):
        ...
        self.processor = ProductVectorProcessor()
        ...
        # Lấy corpus và embeddings từ Qdrant
        self.corpus = self.processor.meta_corpus
        self.embeddings_corpus = self.processor.embeddings_corpus
        
        # Các khởi tạo khác giữ nguyên
        self.tokenized_corpus = [self.split_text(doc["passage"]) for doc in (self.corpus)]
        if len(self.tokenized_corpus) == 0:
            raise ValueError("Corpus is empty. Please ensure the Qdrant collection contains documents.")
        self.bm25 = BM25Okapi(self.tokenized_corpus)
        self.embeddings = SentenceTransformer('bkai-foundation-models/vietnamese-bi-encoder')
    ...
    def retrieve(self, question, topk=50):
        ...
        for i in range(corpus_size):
            ...
            self.corpus[i]["combined_score"] = self.corpus[i]["bm25_normed_score"] * 0.4 + semantic_scores[i] * 0.6
    ...
    def extract_consecutive_subarray(self, numbers):
        """
        Extract consecutive subarrays from a list of numbers
        Ex:  [1,2,3,5,6,8] -> [[1,2,3], [5,6], [8]]
        """
        subarrays = []
        current_subarray = []
        for num in numbers:
